...
class Teacher
{
     ...
     * @param array $filters ตัวกรองข้อมูล (search, department, status, line_status)
     * @return array ข้อมูลครูทั้งหมด
     */
    public function getAllTeachers($limit = 100, $offset = 0, $filters = [])
    {
        try {
            $sql = "SELECT t.*, u.line_id, u.phone_number, u.email, u.first_name, u.last_name, u.role, u.gdpr_consent 
                FROM teachers t
    ...
    // เพิ่มเมธอดอื่นๆ ตามที่จำเป็น เช่น addTeacher, updateTeacher, deleteTeacher, toggleTeacherStatus
    ...
    public function getAllClasses()
    {
        try {
            $query = "SELECT c.class_id, c.level, c.department, c.group_number, 
                             CONCAT(c.level, '/', c.group_number, ' ', c.department) AS class_name
                      FROM classes c
    ...
    // ต่อไปนี้เป็นเมธอดสำหรับจัดการข้อมูลครู
...
/**
 * เพิ่มครูใหม่ (แก้ไขให้มีการตรวจสอบข้อมูลซ้ำก่อน)
 * 
 * @param array $data ข้อมูลครู
 * @return bool สถานะการทำงาน
 */
...
public function addTeacher($data) {
    try {
        ...
        // สร้างข้อมูลครู
        $teacherQuery = "INSERT INTO teachers (user_id, title, national_id, department, position, first_name, last_name, created_at) 
                         VALUES (:user_id, :title, :national_id, :department, :position, :first_name, :last_name, NOW())";
        ...
        $teacherStmt->bindValue(':department', $data['department']);
...
/**
 * อัปเดตข้อมูลครู (แก้ไขให้มีการตรวจสอบข้อมูลซ้ำก่อน)
 * 
 * @param int $teacherId รหัสครู
